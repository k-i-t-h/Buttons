        ...
        <Link className={`text-decoration-none logoText font-weight-bolder text-light mr-4`} to="/">Cardium</Link>  
          ...
          <div className={`navbar-collapse ${!isOpen && "collapse" }`}  id="navbarNavAltMarkup">
              <div className="navbar-nav">
                <Link  className={`MainLinkStyle mr-4 mt-1 ${isOpen && "text-center navItem"}`} to="/" >Home</Link>
                <Link  className={`MainLinkStyle mr-4 mt-1 ${isOpen && "text-center navItem"}`} to="/about" >About</Link>
                <Link  className={`MainLinkStyle mr-4 mt-1 ${isOpen && "text-center navItem"}`} to="/card-deck" >Sample Deck</Link>
                <Link  className={`MainLinkStyle mr-4 mt-1 ${isOpen && "text-center navItem"}`} to="/contact-us" >Contact Us</Link>
                </div>
        </div>
      </nav> 
        )
